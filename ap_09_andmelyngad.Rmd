...
Kõigi tunnuste peale kokku on andmelünki `r andmed %>% is.na() %>% sum()`. 
...
Sellest olulisem on aga teada saada, kui suur on indiviidide osakaal, kellel on vähemalt ühes tunnuses andmelünk.
...
andmed %>% 
  complete.cases() %>%
  `!` %>% # siin võib kasutada ka magrittr::not()
  sum()
...
### Ülevaade lünlikkusest tabelite abil

Uurime lünklikkust veel lähemalt paketi `naniar` funktsioonide abil. Mida näitavad järgnevad kokkuvõtted andmetest?
...
See tabel on ülevaatlikum, kui viime ta pikkformaadist laiformaati.

```{r}
andmed %>% 
  group_by(gndr) %>% 
  miss_var_summary() %>% 
  mutate(gndr = as_factor(gndr)) %>% # et tunnuste nimed oleks laiformaadis selgemad
  pivot_wider(names_from = gndr,
              values_from = c(n_miss, pct_miss))
```

Indiviidide arv ja osakaal lünkade arvu järgi soo lõikes:
...
Pilt ütlevat rohkem kui sada sõna (eeldusel, et oskame pilti adekvaatselt tõlgendada). Teeme siis lünklikkusest kiirema ülevaate saamiseks pilte (st jooniseid). Mida järgneval joonisel kujutatakse?
...
Kuidas saaksime eelneva joonise abil hinnata, kas lünklikkus on seotud näiteks vanusega?
...
Mida järgnevatel joonistel kujutatakse?

```{r}
gg_miss_var(andmed)
gg_miss_var(andmed, show_pct = TRUE)
gg_miss_case(andmed)

andmed %>% 
  mutate(gndr = as_factor(gndr)) %>% 
gg_miss_var(facet = gndr, show_pct = TRUE)

andmed %>% 
  mutate(gndr = as_factor(gndr)) %>% 
gg_miss_fct(fct = gndr)
```
...
Eelnevast analüüsist saime mõningast aimdust, kas lüngad tunnustes võiksid olla täiesti juhuslikud või mitte. **Uurime tunnuste jaotuseid iga tunnuse lünklikkuse lõikes,** kasutades paketi `finalfit` võimalusi. Et analüüs õnnestuks, tuleb siin tunnuste tüübid korrektselt seadistada, st kategoriaalsed tunnused peaksid olema `factor`-tüüpi (arvulised tunnused on meie andmestikus niikuinii `numeric`-tüüpi). Selleks saab kasutada funktsioone `as.factor` või `as_factor`, aga kasutame siin paketist `sjlabelled` funktsiooni `as_label`, mis teeb sama, aga argumendiga `drop.levels = TRUE` jätab välja kategooriad, mis ei esine ühelgi indiviidil (see võimaldab hiljem väljundtabelid kompaktsemalt esitada). Kaasame praegu ainult need tunnused, kus lünklikkus oli suurem kui 1%, sest joonisele ei mahu liiga palju tunnuseid korraga ära, ning taustatunnused, kus küll lünki polnud, kuid nende tunnuste jaotused võivad siiski olulist infot anda.

```{r}
andmed <- andmed %>% 
  mutate(gndr = sjlabelled::as_label(gndr, drop.levels = TRUE),
         brncntr = sjlabelled::as_label(brncntr, drop.levels = TRUE))
tunnused_lünklikkus_vis <- c("lrscale", "imbgeco", "imueclt", "imwbcnt", "rlgdgr", "gndr", 
                              "brncntr", "agea", "eduyrs")
missing_pairs(andmed, 
              explanatory = tunnused_lünklikkus_vis)
```
...
Vaatame lähemalt, **kas vasak-parem-skaalal enesepaigutuse tunnuses esinev lünklikkus on täiesti juhuslik või mitte.** Kasutades paketist `finalfit` funktsiooni `missing_compare`, arvutame teiste arvuliste tunnuste keskmised indiviidide seas, kellel on vasak-parem-skaala tunnuses valiidsed väärtused, ja nende seas, kellel on selles tunnuses andmelünk. Kategoriaalsete tunnuste puhul teeme risttabeli. Lisaks hinnatakse jaotuste erinevust statistiliste testidega. Milliste statistiliste testidega? Uuri funktsiooni abifaili, trükkides konsooli `?missing_compare`.

```{r}
andmed <- andmed %>% 
  mutate(across(imsmetn:impcntr, sjlabelled::as_label, drop.levels = TRUE))
tunnused_lünklikkus_test <- c("imsmetn", "imdfetn", "impcntr", "imbgeco", 
                                "imueclt", "imwbcnt", "rlgdgr", "gndr",
                                "brncntr", "agea", "eduyrs")
missing_compare(andmed, 
                dependent = "lrscale", 
                explanatory = tunnused_lünklikkus_test) %>% 
...
Mida ütlevad hoiatusteated? Tuleta meelde hii-ruut-testi eeldusi.

Mida tulemustest järeldad?

Kui vaadata usklikkust lünkade esinemise suhtes vasak-parem-skaala tunnuses ja nüüd usklikkust lünkade esinemise suhtes immigratsiooni mõju hinnangute tunnuses, siis erinevused usklikkuses on keskmiselt samad. Siiski on eelmises tabelis näha, et nt olulisuse nivool 0.05 oli erinevus statistiliselt oluline, `imbgeco` lünklikkuse puhul aga mitte. Kuidas nii? Kuidas siis peaks neid tulemusi tõlgendada?
...
Samas on ka oluline **hinnata analüüsitavate tunnuste komplekti kui terviku lünklikkust** ehk kas need indiviidid, kellel oli vähemalt ühes tunnuses andmelünk, erinevad kuidagi täielikult mõõdetud indiviididest. See on oluline, sest kui me mingeid omistamismeetodeid ei kasuta, siis tuleb mitmemõõtmelises analüüsis kõik sellised indiviidid analüüsist välja jätta. 
...
Nagu arvata oli, on erinevused üsna selged. Seega ei saa siin rääkida lünkade täielikust juhuslikkusest, mis tähendab, et kui jätaksime lünkadega indiviidid analüüsist lihtsalt välja (mis paraku on vältimatu, kui me ei kasuta mitmest imputeerimist vm sarnast meetodit lünkade täitmiseks), siis tuleks tulemuste tõlgendamisel kindlasti arvesse võtta, kuidas lünkadega indiviidide eemaldamine tulemusi ja tulemuste esinduslikkust võib mõjutada.
